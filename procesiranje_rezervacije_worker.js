...
client.subscribe('pronadi-sobu', async function({ task, taskService }) {
    const rezervacija = task.variables.get('rezervacija');

    const processVariables = new Variables();
    processVariables.set("rezervacija", rezervacija);

    console.log(`Uspješno kreirana rezervacija gosta!`)
...
client.subscribe('stanjeSobe', async function({task, taskService}){
    ...
    console.log(`Promjenjeno stanje sobe ${rezervacija_sobe} u rezervirano!` );
